...
# =========================
# JSON DATA SUPPORT (Stage 2)
# =========================

import json
from sklearn.model_selection import train_test_split

def json_row_to_text_train(example):
    instruction = example.get("instruction", "")
    input_text = example.get("input", "")
    output = json.dumps(example.get("output", {}))
...
def prepare_json_data(dataset_path, process_func, validation_size=0.1, seed=42):
    with open(dataset_path) as f:
        data = json.load(f)

    train_data, val_data = train_test_split(data, test_size=validation_size, random_state=seed)



    train_texts = [process_func(x) for x in train_data]
    val_texts = [process_func(x) for x in val_data]

    train_set = Dataset.from_dict({"text": train_texts})
    val_set = Dataset.from_dict({"text": val_texts})
